...
jest.mock('./ListViewBreadcrumb', () => {
  const ListViewBreadcrumb = () => <div data-testid="breadcrumb">Back to all open responses</div>;
...
jest.mock('./SubmissionsTable', () => {
  const SubmissionsTable = () => <div data-testid="submissions-table">SubmissionsTable</div>;
...
describe('ListView component', () => {
  describe('component', () => {
    ...
    it('displays loading spinner and message when not loaded and no error', () => {
      ...
      expect(screen.getByText('FormattedMessage').previousElementSibling).toHaveClass('pgn__spinner');
    });
    ...
    it('displays EmptySubmission component when loaded but has no submission data', () => {
      ...
      expect(screen.getByTestId('empty-submission')).toBeInTheDocument();
      expect(screen.getByText('Nothing here yet')).toBeInTheDocument();
      expect(screen.getByText('When learners submit responses, they will appear here')).toBeInTheDocument();
    ...
    it('displays ListError component when there is an error', () => {
      ...
      expect(screen.getByRole('button', { name: 'Reload submissions' })).toBeInTheDocument();
      expect(screen.queryByText('FormattedMessage')).not.toBeInTheDocument();
    });
